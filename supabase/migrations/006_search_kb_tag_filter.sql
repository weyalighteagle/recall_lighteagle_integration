-- Phase 5: Add p_allowed_tag_ids filter to search_knowledge_base RPC
-- Applied to: weya-recallai project
-- The && operator checks array overlap — if the meeting has tags [A, B],
-- chunks tagged with either A or B will match (correct behaviour).
-- NULL means no filter (all docs visible). Never pass an empty array.

-- Drop ALL existing overloads to avoid error 42P13
